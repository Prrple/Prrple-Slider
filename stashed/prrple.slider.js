...
(function($){
	...
	function prrpleSliderCreate(root,options){
		...
		var s = {
			...
			add_dots: function(){
				...
				if(s.total <= 1){
					...
					s.el.nav.find(options.el_navdot).each(function(){
						$(this).unbind('click');
						$(this).click(function(){
							if(options.debug){console.log('%c'+options.id+' --- click dot ---','color:#0053A0');};
				...
				if(s.total <= 1){
					...
					if(s.el.left.length > 0){
						...
						s.el.right.unbind('click').click(function(){
							if(options.debug){console.log('%c'+options.id+' --- click right ---','color:#0053A0');};
							s.slide_right();
							return false;
						});
						//left
						s.el.left.unbind('click').click(function(){
							if(options.debug){console.log('%c'+options.id+' --- click left ---','color:#0053A0');};
							s.slide_left();
							return false;
						});
					};
			...
			swipe: {
				...
				add: function(){
					if(typeof($.fn.swipe)==='undefined'){
						//console.log('%c'+options.id+' Please include the jQuery TouchSwipe plugin for swipe gestures.','color:#0053A0');
					}else{
				...
				rich: function(event, phase, direction, distance, orientation, callback){
					if(options.debug){console.log('%c'+options.id+' --- swipe rich - '+phase+' - '+direction+' - '+distance+' - '+orientation+' ---','color:#0053A0');};
					if(s.total>1){
						if(phase=='start'){
			...
			//REMOVE SLIDER
			remove: function(){
				if(options.debug){console.log('%c'+options.id+' remove','color:#0053A0');};
				...
				s.el.nav.html('');
				if(options.addDots){
					s.slider.find(options.el_nav).remove();
				};
				if(options.addArrows){
					s.slider.find(options.el_left);
					s.slider.find(options.el_right).remove();
				}
				//REMOVE CSS
				console.log(s.slider.find('*'));
